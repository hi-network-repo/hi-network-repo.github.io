 ...
 * @require 1
 *
 * @param murasakiImageName
 * @text 紫っこの画像ファイル名
 * @desc 紫っこの改行カーソルとして表示する画像ファイルです。
 * 24px～36pxの正方形サイズを推奨します。
 * @dir img/system
 * @type file
 * @require 1
...
(() => {
    ...
    const murasakiImageName = String(parameters["murasakiImageName"] || "");
    ...
    var murasakiEnable = false;

    PluginManager.registerCommand(pluginName, "murasakiOn", function (args) {
        murasakiEnable = true;
    });

    PluginManager.registerCommand(pluginName, "murasakiOff", function (args) {
        murasakiEnable = false;
    });
    ...
    Window_Message.prototype._refreshPauseSign = function() {
        ...
        else if (murasakiEnable) {
            this._pauseSignSprite.bitmap = ImageManager.loadSystem(murasakiImageName);
        }
        else {
