...
(function() {
    ...
    class PictureTouch {
        ...
        updatePictureTouch() {
            if ($gameSwitches.value(256)==false) {
        ...
        isOnPicturePos(x = TouchInput.x, y = TouchInput.y) {
            ...
            const eventData = $gameScreen.findPictureEvent(this._pictureId, 1);
            //console.log(eventData.ignoreAlpha)
            if (eventData && eventData.ignoreAlpha) {
                if (bx >= 0 && by >= 0 && bx <= pic.bitmap.width && by <= pic.bitmap.height) {
                    return true;
                }
            }
        ...
        isTouchEvent(triggerFunc) {
            return triggerFunc.call(TouchInput) && this.isOnPicturePos(TouchInput.x, TouchInput.y,1);
        }
