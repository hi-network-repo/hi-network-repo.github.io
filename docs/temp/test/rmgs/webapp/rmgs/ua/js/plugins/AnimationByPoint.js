...
(() => {
    ...
    Sprite_Destination.isReqested = false;

    const _Sprite_DestinationUpdate = Sprite_Destination.prototype.update;
    Sprite_Destination.prototype.update = function () {
        if ($gameTemp.isDestinationValid()) {
            if (this.isReqested == false) {
                const point = new Game_AnimationPoint();
                $gameTemp.requestAnimation([point], 3);
    ...
    class Game_AnimationPoint extends Point {
        constructor() {
            //super(args.x, args.y);
            super(TouchInput.x, TouchInput.y);
            this._wait = false;//args.wait;
            this._scroll = true;//args.scroll;
        }
